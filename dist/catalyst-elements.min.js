...
a.join(" "))}else this.removeAttribute("aria-labelledby");this._selectElement.disabled=this.disabled;this.notifySelectedOptionChanged()}}_setUpDimensions(){this.style.minWidth="";this.style.minHeight="";let a=getComputedStyle(this);var d=Number.parseInt(a.width,10),b=Number.parseInt(a.height,10);d=0===d;b=0===b;if(d||b){let c=0,e=0;this._cardElement.style.position="relative";let f=this._selectElement.options;if(0<f.length){this._cardFrontFace.style.position="relative";this._cardBackFace.style.position=
"relative";let a=this._cardFrontFace.textContent;this._cardFrontFace.textContent="";let h=this._cardBackFace.textContent;this._cardBackFace.textContent="";for(let a=0;a<f.length;a++){this._cardFrontFace.textContent=f[a].textContent;this._cardBackFace.textContent=f[a].textContent;if(d){var g=this._cardFrontFace.offsetWidth;g>c&&(c=g);g=this._cardBackFace.offsetWidth;g>c&&(c=g)}b&&(g=this._cardFrontFace.offsetHeight,g>e&&(e=g),g=this._cardBackFace.offsetHeight,g>c&&(c=g))}this._cardFrontFace.textContent=
a;this._cardBackFace.textContent=h;this._cardFrontFace.style.position="";this._cardBackFace.style.position=""}this._cardElement.style.position="";d&&(d=c+2,this.style.minWidth=d+"px",this.noAutoPerspective||(this.style.perspective=2*(d+Number.parseInt(a.paddingLeft,10)+Number.parseInt(a.paddingRight,10))+"px"));b&&(this.style.minHeight=e+2+"px")}}disconnectedCallback(){this.removeEventListener("keydown",this._onKeyDown);this.removeEventListener("click",this._onClick);this.removeEventListener("mouseUp",
...
b);b?this.inputElement.setAttribute("required",""):this.inputElement.removeAttribute("required");break;case "name":this.inputElement.setAttribute("name",new String(c));break;case "value":this.inputElement.setAttribute("value",new String(c));break;case "form":this.inputElement.setAttribute("form",c)}}_toggleChecked(){this.disabled||(this.checked=!this.checked,this.dispatchEvent(new CustomEvent("change",{detail:{checked:this.checked},bubbles:!0})))}_onClick(){this._toggleChecked()}_onKeyDown(a){if(!a.altKey)switch(a.keyCode){case b._KEYCODE.SPACE:case b._KEYCODE.ENTER:a.preventDefault(),
this._toggleChecked()}}}}]);}).call(this)
