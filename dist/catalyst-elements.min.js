...
a.join(" "))}else this.removeAttribute("aria-labelledby");this._selectElement.disabled=this.disabled;this.notifySelectedOptionChanged()}}_setUpDimensions(){this.style.minWidth="";this.style.minHeight="";let a=getComputedStyle(this);var b=Number.parseInt(a.width,10),d=Number.parseInt(a.height,10);b=0===b;d=0===d;if(b||d){let c=0,h=0;this._cardElement.style.position="relative";let e=this._selectElement.options;if(0<e.length){this._cardFrontFace.style.position="relative";this._cardBackFace.style.position=
"relative";let a=this._cardFrontFace.textContent;this._cardFrontFace.textContent="";let g=this._cardBackFace.textContent;this._cardBackFace.textContent="";for(let a=0;a<e.length;a++){this._cardFrontFace.textContent=e[a].textContent;this._cardBackFace.textContent=e[a].textContent;if(b){var f=this._cardFrontFace.offsetWidth;f>c&&(c=f);f=this._cardBackFace.offsetWidth;f>c&&(c=f)}d&&(f=this._cardFrontFace.offsetHeight,f>h&&(h=f),f=this._cardBackFace.offsetHeight,f>c&&(c=f))}this._cardFrontFace.textContent=
a;this._cardBackFace.textContent=g;this._cardFrontFace.style.position="";this._cardBackFace.style.position=""}this._cardElement.style.position="";b&&(b=c+2,this.style.minWidth=b+"px",this.noAutoPerspective||(this.style.perspective=2*(b+Number.parseInt(a.paddingLeft,10)+Number.parseInt(a.paddingRight,10))+"px"));d&&(this.style.minHeight=h+2+"px")}}disconnectedCallback(){this.removeEventListener("keydown",this._onKeyDown);this.removeEventListener("click",this._onClick);this.removeEventListener("mouseUp",
...
a.borderTopWidth);this.style.setProperty("--catalyst-toggle-switch-bar-border-left-wdith",a.borderLeftWidth);0>Number.parseFloat(getComputedStyle(this).getPropertyValue("--catalyst-toggle-switch-knob-offset"))&&this._bar.classList.add("negitive-knob-offset")}}}]);}).call(this)
